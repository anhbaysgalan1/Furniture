...
module.exports = {
   ...
   * @param fileName tên file x ra.
   * @param headers mảng chứa thông tin các cột {key: 'userCode', value: 'Mã NV', width: 15}
   *                key: để lấy dữ liệu trong mảng listData
   *                value: text hiển thị
   *                width: độ rộng của cột
   * @param listData dữ liệu xuất exceluất
   * @param options
   */
  exportExcel(fileName, headers, listData, options) {
    let today = moment().format('YYYY-MM-DD');
    let relativePath = Env.BackendUploadFolder + '/files/tmp/' + today + '/';   // tạo folder theo ngày
    let fullPath = Env.BackendUploadBasedir + relativePath;
    try {
