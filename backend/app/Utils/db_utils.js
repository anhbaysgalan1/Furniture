...
const db_conn_str = env.DBMongoConnectionString;
const dbMongo = mongo.connect(db_conn_str);
const TAG = 'DBUtils';
...
module.exports = {
  ...
  isTestUser: function (user) {
    return user.isTest && user.isTest.toString().toLowerCase() === "true";
  },
  ...
  getListPagination: async function (dbCollection, skip, limit, sort, aggregateArray) {
    ...
    return new Promise((resolve, reject) => {
      (async () => {
        ...
        let data = Utils.paginationResult(skip, limit, result[0], result[1]);
        resolve(data);
      })();
    });
  },
