...
import View from "views/Contact/ListContact"
...
class Index extends BaseContainer {
  ...
  onSubmit(values) {
    this.props
      .dispatch(ContactAction.edit({ _id: this.id, ...values }))
      .then(data => {
        if (!data.error) {
          ...
          switch (err.status) {
            ...
            case 404: {
              if (err.message === "Permission_Not_Exist") {
                this.notify(
                  I18n.t("Backend.Role.Permission_Not_Exist"),
                  "error"
                )
              }
              break
  ...
  componentDidMount() {
    this.props.dispatch(ContactAction.fetchAll({ pageSize: -1 }))
  }

  onFetchData(state) {
    this.props.dispatch(ContactAction.fetchAll(state))
  }
  ...
  onDeleteData(selectedIds) {
    this.props
      .dispatch( ContactAction.delete({ ids: selectedIds }) )
      .then(result => {
        if (!result.error) {
          //success
...
const mapStateToProps = state => {
  return {
    data: selector(state, 'contact.list', {}) ,
  }
}
